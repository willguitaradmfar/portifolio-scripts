...
export class InsertStockPortifolio implements InsertStock {
    ...
    async execute(list: Array<{ code: string; quantity: number; average_closing_price: number; wallet_code: string }>): Promise<any> {
        if (!list.length) {
        ...
        let toInserts = await Promise.all(list.map(async ({ code, quantity, average_closing_price, wallet_code }) => {
            if (!code) {
                throw new RuntimeError(
                    "Code is required",
                    "ERROR_CODE_REQUIRED",
